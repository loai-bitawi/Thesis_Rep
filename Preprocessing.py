...
class preprocessing():
    ...
    def data_save(self,main_path,doc_words, words,doc_vecs, vec_qr,rel_ass):
        ...
        with jsonstreams.Stream(jsonstreams.Type.OBJECT, filename= main_path+'docs.txt') as s:
             s.write('dict',doc_vecs)
    ...
    def lisa_preprocessing(self,sample=False,sample_size=-1,stop_words=True):
        main_path=self.defaults.main_path+'Lisa/'
        docs,queries,rel_ass= self.parser.lisa_parser()    
        doc_words, words,doc_vecs, vec_qr= self.embedder(docs,queries,rel_ass,stop_words)
    ...
    def aila_preprocessing(self,sample=False,sample_size=-1,stop_words=True):
        ...
        if sample:
            self.sampler(docs,rel_ass,doc_vecs,main_path,size=sample_size)
    def cran_preprocessing(self,sample=False,sample_size=-1,stop_words=True):
